...
static bool mkfs(void *image, size_t size, mkfs_opts *opts)
{
	...
	int num_blocks = div_round_up(opts->n_inodes, inodes_per_block);
	for(int i = VSFS_ITBL_BLKNUM; i <= VSFS_ITBL_BLKNUM + num_blocks; ++i){
	...
	root_entries = (vsfs_dentry *)(image + (VSFS_ITBL_BLKNUM + num_blocks) * VSFS_BLOCK_SIZE);
	root_ino->i_mode = S_IFDIR | 0777;
	root_ino->i_blocks = 1;
	root_ino->i_nlink = 2;
	root_ino->i_direct[0] = VSFS_ITBL_BLKNUM + num_blocks;
	...
	for(long unsigned int i = 2;i < VSFS_BLOCK_SIZE / sizeof(vsfs_dentry);++i){
	...
	sb->sb_free_blocks = nblks - VSFS_ITBL_BLKNUM - num_blocks - 1;
	sb->sb_free_inodes = opts->n_inodes - 1;
	sb->sb_data_region = VSFS_ITBL_BLKNUM + num_blocks;
	sb->sb_size = size;
